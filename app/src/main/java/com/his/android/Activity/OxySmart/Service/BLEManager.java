...
public class BLEManager {	
	private static final String TAG = "frf";//"BLEManager";
...
//			}
			...
			((Activity)mContext).runOnUiThread(new Runnable() {
                @Override
                public void run() {
                	if(OxySmartActivity.mLeDeviceListAdapter!=null
                			/*&& device.getName()!=null
                			&& device.getName().contains("PC-60F")*/){
						OxySmartActivity.mLeDeviceListAdapter.addDevice(device);
						OxySmartActivity.mLeDeviceListAdapter.notifyDataSetChanged();
                	}
                }
            });
		}
	...
	//test
	public void myBLEConnect(BluetoothDevice device){
		mTargetDevice = device;
		scanLeDevice(false);
		synchronized (this) {
			bBindServ = true;
